(ns civileng.actor
  "CivilEngineersActor — the ISCO-08 2142 community civileng actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one civileng operation request
  ...
  The unconditional invariant: the CivilEngineersAdvisor can never
  directly commit a record the CivilEngineersGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [civileng.advisor :as advisor]
            [civileng.governor :as governor]
            [civileng.store :as store]))

(defn build-graph
  "Build a compiled CivilEngineersActor graph. `store` implements
  `civileng.store/Store`. `advisor` implements
  `civileng.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :structure-id (:structure-id proposal)
                                    :payload proposal}]
